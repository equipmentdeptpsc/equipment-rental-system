...
import { canUseCanonicalRemoteRentalApprovalMutations, canUseCanonicalRemoteRentalMutations, canUseCanonicalRemoteRentalReleaseMutation, canUseCanonicalRemoteRentalReserveMutation, canUseLegacyRentalMutations } from "../services/rentalRuntimeCapability";
...
export default function RentalQuickActions({ rental, hideClose = false }: { rental: RentalRecord; hideClose?: boolean }) {
  ...
  const mutationsAvailable = legacyMutations || canonicalOperationalMutations || canonicalApprovalMutations || canonicalReserveMutations || canonicalReleaseMutations;
  ...
  const model = canonicalOperationalMutations || canonicalApprovalMutations || canonicalReserveMutations || canonicalReleaseMutations
    ? rental.status === "Draft"
  ...
  async function run(id: RentalQuickActionId) {
    ...
    if (canonicalOperationalMutations || canonicalApprovalMutations || canonicalReserveMutations || canonicalReleaseMutations) {
      ...
      try {
        ...
        else if (id === "activate" && canonicalOperationalMutations) result = await repository.activate(input);
